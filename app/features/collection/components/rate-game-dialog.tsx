  ...
  return (
    <Dialog open={isRateGameDialogOpen} onOpenChange={setIsRateDialogOpen}>
      <DialogContent>
        ...
        <rateFetcher.Form className="flex flex-col gap-3" method="put" action={`/api/collections/${userId}`}>
          <div className="w-full py-2 text-center">{rating}</div>
          <input type="hidden" name="gameId" value={gameId} />
          <Slider name="rating" value={[rating]} onValueChange={(v) => setRating(v[0])} />
          <Button type="submit" onClick={() => setIsRateDialogOpen(false)} variant={"outline"}>
            Submit
          </Button>
        </rateFetcher.Form>
      </DialogContent>
